	...
	async run(client, msg, args, prefix) {
		...
		if (msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			if (member.user.id == msg.author.id)
				return msg.channel.send(":x: Вы не можете забанить самого себя.");
			if (member.user.id == client.user.id)
				return msg.channel.send(":x: Вы не можете забанить бота.");
			if (member.hasPermission("ADMINISTRATOR"))
				return msg.channel.send(":x: Я не могу забанить этого пользователя, так как он является администратором на этом сервере.");
